...
app.use(
  cors({
    origin: ["http://localhost:3000", "https://mindlog-ai.vercel.app"],
  }),
);
...
app.post("/api/journal", (req, res) => {
  ...
  if (!validAmbiences.includes(ambience.toLowerCase())) {
    return res
      .status(400)
      .json({ error: `ambience must be one of: ${validAmbiences.join(", ")}` });
  }
...
app.post("/api/journal/analyze", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "LLM analysis failed.", detail: err.message });
  }
});
...
app.listen(PORT, () =>
  console.log(`MindLog AI backend running on http://localhost:${PORT}`),
);
